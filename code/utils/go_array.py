...
def nii_to_1d(nii_dict, h5_path, nameit, normalization=True, smooth=None, tf=False):
    ...
    for i in nii_dict.keys():
        ...
        for j in sub_files_list:
            ...
            # This one passed!
            # print("-" * 80)
            # print("check point go!!!")
            # print(np.max(nii_flat))
            # print(np.mean(nii_flat))
            #
            # print("-" * 80)
    ...
    ####
    print ("check point 0:")
    print (go_over_list[:3])
    check = output_x[:3, :]
    print(check.shape)
    for i in range(3):
        print(np.max(check[i, :]))
        print(np.mean(check[i,:]))
