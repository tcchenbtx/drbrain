...
model_body = os.path.join(model_path, "V3_3D_model.h5")
model_weights = os.path.join(model_path, "V3_3D_weights.h5")
...
##############
x = conv3d_bn(x, 80, 1, 1, 1, border_mode='valid', name='final_conv')
#x = conv3d_bn(x, 64, 3, 3, 3, border_mode='valid')
...
sgd = SGD(lr=0.0001, decay=1e-6, momentum=0.9, nesterov=True) # lr=0.01
...
#
score_valid = model.evaluate_generator(data_generator.generator_tf(unique_tf_valid_path),
                                       val_samples=32, max_q_size=1, nb_worker=1, pickle_safe=False)

score_test = model.evaluate_generator(data_generator.generator_tf(unique_tf_test_path),
                                      val_samples=215, max_q_size=1, nb_worker=1, pickle_safe=False)
...
print('Valid score:', score_valid[0])
print('Valid accuracy:', score_valid[1]*100)
